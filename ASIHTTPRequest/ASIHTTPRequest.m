...
- (void)updateDownloadProgress {
  ...
  if (bytesReceivedBlock) {
    ...
    [self performBlockOnMainThread:^{
      if (bytesReceivedBlock) {bytesReceivedBlock(value, totalSize);}
    }];
  }
#endif
...
- (void)updateUploadProgress {
  ...
  if (bytesSentBlock) {
    ...
    [self performBlockOnMainThread:^{
      if (bytesSentBlock) {bytesSentBlock(value, totalSize);}
    }];
  }
#endif
}
...
- (void)incrementDownloadSizeBy:(long long)length {
  ...
  if (downloadSizeIncrementedBlock) {
    [self performBlockOnMainThread:^{
      if (downloadSizeIncrementedBlock) {downloadSizeIncrementedBlock(length);}
    }];
  }
#endif
}
...
- (void)incrementUploadSizeBy:(long long)length {
  ...
  if (uploadSizeIncrementedBlock) {
    [self performBlockOnMainThread:^{
      if (uploadSizeIncrementedBlock) {uploadSizeIncrementedBlock(length);}
    }];
  }
#endif
}
...
- (void)removeUploadProgressSoFar {
  ...
  if (bytesSentBlock) {
    unsigned long long totalSize = [self postLength];
    [self performBlockOnMainThread:^{
      if (bytesSentBlock) {bytesSentBlock(progressToRemove, totalSize);}
    }];
  }
#endif
}
...
+ (void)registerForNetworkReachabilityNotifications {
  ...
  [[ASIReachability reachabilityForInternetConnection] startNotifer];
