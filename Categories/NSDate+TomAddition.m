...
- (NSUInteger)pd_weekDay {
  ...
  //加上NSLocale，星期一为一个星期的第一天，int返回为1
//  NSLocale *deLocale = [[[NSLocale alloc] initWithLocaleIdentifier:@"de_DE"] autorelease];
...
+ (NSString*)pd_timeIntervalToHHMMSS:(long long)oriSecond {
  
  NSMutableString *mulString=[[[NSMutableString alloc]init]autorelease];
  ...
  if (oriSecond >= 60*60){
    ...
    [mulString appendString:[NSString stringWithFormat:@"%@:",[Utilities stringWithZero:oriSecond/(60*60)]]];
  } else{
    
    [mulString appendString:@"00:"];
  }
  int min=oriSecond%(60*60);
  if ( min>= 60){
    
    [mulString appendString:[NSString stringWithFormat:@"%@:",[Utilities stringWithZero:min/60]]];
  ...
  int second=min%60;
	if (second >= 0) {
		[mulString appendString:[NSString stringWithFormat:@"%@",[Utilities stringWithZero:second]]];
	} else{
    [mulString appendString:@"00"];
  }
...
- (BOOL)pd_isWeekend {
  ...
  if ([self pd_weekDay] == 7 || [self pd_weekDay] == 1) {
    return YES;
  }
  return NO;
}
