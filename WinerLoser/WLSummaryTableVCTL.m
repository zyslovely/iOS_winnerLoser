...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    if ([indexPath section]== 0) {
        ...
        {
            //增加玩家
            UIAlertView *alert = [[UIAlertView alloc]initWithTitle:@"输入好友名称" message:@"" delegate:self cancelButtonTitle:@"取消" otherButtonTitles:@"确定",nil];
            alert.tag = kAlertAdd;
        ...
        if ([indexPath row] == 3) {
            ...
            {
                WLDeleteUserVCTL *deleteUserVCTL = [[WLDeleteUserVCTL alloc] init];
                [self.navigationController pushViewController:deleteUserVCTL animated:YES];
                [deleteUserVCTL release];
            }
...
- (void)alertView:(UIAlertView *)alertView clickedButtonAtIndex:(NSInteger)buttonIndex {
    ...
    if (alertView.tag == kAlertAdd)
    {
        if ([[alertView buttonTitleAtIndex:buttonIndex] isEqualToString:@"确定"])
        {
            if ([_addTextField.text isEmpty])
            {
                return;
            }
            WLdbUserObj *user = [[WLdbUserObj alloc] init];
            user.user_name = _addTextField.text ;
            [user saveToDB];
            [[[WLAppDelegate sharedDelegate] gameInfo].attendees addObject:user];
